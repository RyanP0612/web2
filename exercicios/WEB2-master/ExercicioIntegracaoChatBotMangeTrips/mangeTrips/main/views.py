...
class ConversationHistoryViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return ConversationHistory.objects.filter(user_id=user_id)
...
class ConversationViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Conversation.objects.filter(history_id=history_id)
...
def convertToMessage(data, attributeName):
    ...
    response = ' \n '
    for dt in data:
        attributeValue = getattr(dt, attributeName, None)
        if attributeValue is not None:
            if attributeName == 'tripFK':
                response += str(index) + ' - ' + str(attributeValue) + '-' + str(dt.date) + ' \n '
                index += 1
            elif attributeName == 'link':
                messages.append(attributeValue)
                return messages
            else:
                response += str(index) + ' - ' + attributeValue + ' \n '
                index += 1
    return response
...
class ChatBotAPIView(APIView):
    def post(self, request):
        ...
        while conversationFound.lastCommand == 'SEARCH_TRIP':
            ...
            if answer.command == 'RETURN':
                conversationFound.lastCommand = "RETURN"
        ...
        elif answer.command == 'SEARCH_AVAIBILITY':
            availability = Availability.objects.all()
            finalMessage = convertToMessage(availability, 'tripFK')
            newAnswer = Conversation(type="A",message=finalMessage if answer.additionalMessage is None else finalMessage + '\n' + answer.additionalMessage ,history=conversationFound)
        elif answer.command == 'GET_IMAGE':
                img = Image.objects.all()
                finalMessage = convertToMessage(img, 'link')
                newAnswer = Conversation(type="A", message=finalMessage if answer.additionalMessage is None else finalMessage + '\n' + answer.additionalMessage, history=conversationFound)
